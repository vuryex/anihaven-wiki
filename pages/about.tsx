...
const AboutPage: React.FC<PageProps> = ({ content }) => {
  if (!content) {
    return (
      <>
        <Head>
          <title>About Us - AniHaven Wiki</title>
          <meta name="description" content="Learn about the AniHaven community" />
        </Head>
        
        <div className="text-center py-12">
          <h1 className="text-3xl font-bold text-text-primary mb-4">About Us</h1>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  try {
    ...
    content = getMarkdownContent('about');
  } catch (error) {
